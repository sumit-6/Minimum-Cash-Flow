...
void PlotInput(vector<vector<int>>& graph)
{
    ...
    Mat _img(1000, 1500, CV_8UC3, BackGroundColour);
    int boundaryRadius = 200;
    int shiftX = 600, shiftY = 350;
    double TWO_PI = (double)2 * PI;
    int d = (((double)20 / TWO_PI)*atan(N)) + 5;
    int Noderadius = ((TWO_PI * boundaryRadius) / N) - d;
    Noderadius /= 4;
    int i = 1;
    double fontSize = ((double)2.9 * Noderadius) / 76, theta = 0.0;
    for (int k = 0;k < N; k++)
    {
        ...
        circle(_img, Point(x, y), Noderadius , Scalar(0, 0, 0));
        string p = to_string(i);
        i++;
        putText(_img, p, Point(x - (Noderadius/1.5), y + Noderadius/4), FONT_HERSHEY_SIMPLEX, fontSize, Scalar(0, 69, 255), 1);
        theta += TWO_PI / N;
    }
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            if (graph[i][j] != 0)
            {
                ...
                x2 = (((D- Noderadius)*(x0)) + (Noderadius*x1))/D;
                y2 = (((D - Noderadius) * (y0))+(Noderadius * y1)) / D;

                x3 = (((D - Noderadius) * (x1)) + (Noderadius * x0)) / D;
                y3 = (((D - Noderadius) * (y1)) + (Noderadius * y0)) / D;
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (signX == 0)
                    {
                        X0 = x3 + (Noderadius / 3) * (cos(angle + ((double)30 * PI) / 180));
                        X1 = x3 + (Noderadius / 3) * cos(angle - ((double)30 * PI) / 180);
                        Y0 = y3 + (Noderadius / 3) * (sin(angle + ((double)30 * PI) / 180));
                        Y1 = y3 + (Noderadius / 3) * sin(angle - ((double)30 * PI) / 180);
                    }
                    else
                    {
                        X0 = x3 - (Noderadius / 3) * (cos(angle + ((double)30 * PI) / 180));
                ...
                arr[i] = arr[i] + (Noderadius / 2.5);
                putText(_img, payment, Point(x0 - (Noderadius / 2),y0 + Noderadius + arr[i]), FONT_HERSHEY_SIMPLEX, fontSize/4, Scalar(0, 0, 0), 1.8);
            }
        }
    }
    imshow("Image", _img);
